...
export class BenefitService {
  ...
  {
    let deceasedRetirementBenefit: number = 0 // 
    let survivorBenefit: number

    let survivorBenefitPIA: number // PIA to use for calculating survivor benefits
    let wepAdjustmentFactor: number
    let pia825: number // can set lower limit of survivor benefit
    let wepAffected: boolean // deceased was affected by Windfall Elimination Provision

    if (deceasedPerson.wepSurvivorPIA == 0) {
      // deceasedPerson was not affected by WEP
      survivorBenefitPIA = deceasedPerson.PIA
      wepAdjustmentFactor = 1
      wepAffected = false
    } else {
      // deceasedPerson was affected by WEP
      survivorBenefitPIA = deceasedPerson.wepSurvivorPIA
      wepAdjustmentFactor = survivorBenefitPIA / deceasedPerson.PIA
      wepAffected = true
    }
    pia825 = 0.825 * survivorBenefitPIA

    //If deceasedPerson had filed, survivorBenefit = deceasedPerson's retirement benefit (with no WEP reduction), but no less than 82.5% of deceased's PIA
    if (deceasedClaimingDate <= dateOfDeath) {
      ...
      if (wepAffected) {
        deceasedRetirementBenefit = deceasedRetirementBenefit * wepAdjustmentFactor
      }

      survivorBenefit = deceasedRetirementBenefit
      if (survivorBenefit < pia825) {
        survivorBenefit = pia825
      }
    }
    else { //i.e., deceasedPerson had NOT filed as of date of death...
      //if deceasedPerson was younger than FRA, survivor benefit = deceasedPerson's survivorBenefitPIA
      if (dateOfDeath < deceasedPerson.FRA) {
        survivorBenefit = survivorBenefitPIA
      }
      //if deceased spouse was older than FRA, survivorBenefit = retirement benefit deceased could have received on date of death, adjusted if reduced for WEP
      else {
        survivorBenefit = this.calculateRetirementBenefit(deceasedPerson, dateOfDeath)
        if (wepAffected) {
          survivorBenefit = survivorBenefit * wepAdjustmentFactor
        }
      }
    }
    ...
    if (survivorSurvivorBenefitDate < survivingPerson.survivorFRA) {
      ...
      if (deceasedClaimingDate >= deceasedPerson.FRA) { //deceased had not filed before FRA
        //adjust survivor benefit downward relative to initial survivor benefit calculation above.
        survivorBenefit = survivorBenefit - (survivorBenefit * 0.285 * (1 - percentageWaited))
      } else { //deceased had filed before FRA
        // do completely new calculation, with survivor benefit based on survivorBenefitPIA rather than deceased retirement benefit.
        survivorBenefit = survivorBenefitPIA - (survivorBenefitPIA * 0.285 * (1 - percentageWaited))
        //survivorBenefit then limited to greater of
        // (A) 82.5 % of deceased's PIA or
        // (B) amount deceased was receiving on date of death or would have received, if not affected by WEP
        if (pia825 < deceasedRetirementBenefit) {
