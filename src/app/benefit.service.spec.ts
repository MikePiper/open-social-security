...
describe('BenefitService', () => {
  ...
  it('should calculate retirement benefit 60 months early as 70% of PIA', inject([BenefitService], (service: BenefitService) => {
    let FRA:Date = new Date (2030, 7, 1) //FRA Aug 1, 2020
    let benefitDate = new Date (2025, 7 , 1) //Benefit date 5 years prior to FRA
    expect(service.calculateRetirementBenefit(1000, FRA, benefitDate))
        .toEqual(700)
  }))
  ...
  it('should calculate retirement benefit 36 months early as 80% of PIA', inject([BenefitService], (service: BenefitService) => {
    let FRA:Date = new Date (2030, 7, 1) //FRA Aug 1, 2020
    let benefitDate = new Date (2027, 7 , 1)
    expect(service.calculateRetirementBenefit(1000, FRA, benefitDate))
        .toEqual(800)
  }))
  ...
  it('should calculate retirement benefit 12 months early as 93.33% of PIA', inject([BenefitService], (service: BenefitService) => {
    let FRA:Date = new Date (2030, 7, 1) //FRA Aug 1, 2020
    let benefitDate = new Date (2029, 7 , 1)
    expect(service.calculateRetirementBenefit(1000, FRA, benefitDate))
        .toBeCloseTo(933.33, 1)
  }))
  ...
  it('should calculate spousal benefit as zero when own PIA is too high', inject([BenefitService], (service: BenefitService) => {
    let PIA: number = 1000
    let otherSpousePIA: number = 1500
    let FRA:Date = new Date (2030, 7, 1) //FRA Aug 1, 2020
    let retirementBenefit: number = 800
    let spousalStartDate = new Date (2027, 7 , 1)
    let governmentPension: number = 0
    expect(service.calculateSpousalBenefit(PIA, otherSpousePIA, FRA, retirementBenefit, spousalStartDate, governmentPension))
        .toEqual(0)
  }))
    ...
    it('should calculate spousal benefit appropriately after FRA', inject([BenefitService], (service: BenefitService) => {
      let PIA: number = 800
      let otherSpousePIA: number = 2000
      let FRA:Date = new Date (2030, 7, 1) //FRA Aug 1, 2020
  ...
  //Testing calculateSurvivorBenefit
    it('should calculate survivor benefit appropriately after FRA, with own smaller retirement benefit. Deceased filed at age 70, died at 71', inject([BenefitService], (service: BenefitService) => {
      let survivorSSbirthDate: Date = new Date (1963, 7, 1) //SSbirthday Aug 1, 1963
      let survivorSurvivorFRA: Date = new Date (2030, 7, 1) //FRA Aug 1, 2020
      ...
      let deceasedFRA: Date = new Date (2030, 2, 1) //deceased FRA March 1, 2030
      let dateOfDeath: Date = new Date (2034, 2, 1) //deceased died at 71
      let deceasedPIA: number = 1000
      let deceasedClaimingDate: Date = new Date (2033, 2, 1) //deceased filed 3 years after FRA
      let governmentPension: number = 0
      expect(service.calculateSurvivorBenefit(survivorSSbirthDate, survivorSurvivorFRA, survivorRetirementBenefit, survivorSurvivorBenefitDate, deceasedFRA, dateOfDeath, deceasedPIA, deceasedClaimingDate, governmentPension))
    ...
    it('should calculate survivor benefit appropriately before FRA, with own smaller retirement benefit. Deceased filed at age 70, died at 71', inject([BenefitService], (service: BenefitService) => {
      ...
      let deceasedFRA: Date = new Date (2020, 11, 1) //deceased FRA December 1, 2020 (was born in Dec 1954 and has FRA of 66)
