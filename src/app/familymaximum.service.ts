    ...
    if (scenario.children.length > 0 || personA.isOnDisability === true || personB.isOnDisability === true){//Only bother with all this if we know familymax might be applicable (because of kids or a person being disabled)
      ...
      //Find out whether to use personA's family max, personB's family or, or combined family max
        ...
        if(personA.entitledToRetirement === true && personB.entitledToRetirement === true && entitledChild === true){//if there is a child entitled on both personA and personB
          if (personA.retirementBenefitDate > this.today || personB.retirementBenefitDate > this.today){//if one retirementBenefitDates is in the future, use this year as simultaneous entitlement year (becuase we don't know future year's bend points)
