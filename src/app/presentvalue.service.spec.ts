...
describe('PresentValueService', () => {
  ...
  it('should tell a single person to file ASAP with very high discount rate', inject([PresentValueService], (service: PresentValueService) => {
    ...
    let mortalityService:MortalityService = new MortalityService()
    let mortalityTable:number[] = mortalityService.determineMortalityTable ("male", "SSA", 0)
    expect(service.maximizeSinglePersonPV(maritalStatus, PIA, SSbirthDate, actualBirthDate, initialAge, FRA, quitWorkDate, monthlyEarnings, mortalityTable, discountRate).solutionsArray[0].date)
  ...
  it ('should tell a high-PIA spouse to wait until 70, with low discount rate and long lifespans', inject([PresentValueService], (service: PresentValueService) => {
    ...
    spouseAFRA, spouseBFRA, spouseAsurvivorFRA, spouseBsurvivorFRA, spouseAmortalityTable, spouseBmortalityTable, spouseAquitWorkDate, spouseBquitWorkDate, spouseAmonthlyEarnings, spouseBmonthlyEarnings, spouseAgovernmentPension, spouseBgovernmentPension, discountRate).solutionsArray[1].date)
